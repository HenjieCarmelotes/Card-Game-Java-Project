...
public class GreaterSmaller {
	...
	public static void main(String[] args) {
		...
		System.out.println("GreaterSmaller. A card is dealt from a deck of cards.");
		System.out.println("You have to predict whether the next card will be");
		System.out.println("greater or smaller. Your score in the game is the");
	...
	/**
	* Lets the user play one game of GreaterSmaller, and returns the
	...
	*/
	private static int play() {
		...
		Card nextCard; // The next card in the deck. The user tries to predict whether this is greater or smaller than the current card.
		int correctGuesses ; // The number of correct predictions the
							// user has made. At the end of the game,
							// this will be the user’s score.
		char guess; // The user’s guess. ’G’ if the user predicts that
					// the next card will be greater, ’S’ if the user
					// predicts that it will be smaller.
		...
		while (true) { // Loop ends when user’s prediction is wrong.
			...
			/* Get the user’s prediction, ’G’ or ’S’ (or ’g’ or ’s’). */	
		
			System.out.print("Will the next card be greater (G) or smaller (S)? ");
			do {
				Scanner stdin1 = new Scanner (System.in);
				guess = stdin1.next().charAt(0);
				guess = Character.toUpperCase(guess);
				if (guess != 'G' && guess != 'S')
					System.out.print("Please respond with G or S: ");
			} while (guess != 'G' && guess != 'S');
