...
public class Deck {
    ...
    public Deck(boolean includeJokers) {
        ...
        for ( int suit = 0; suit <= 3; suit++ ) {
            for ( int value = 1; value <= 13; value++ ) {
                deck[cardCt] = new Card(value,suit); //sets the value and suit of each position of the card.
                cardCt++;
            }
        }
        if (includeJokers) {
    ...
    public void shuffle() {
        for ( int i = deck.length-1; i > 0; i-- ) {
            int rand = (int)(Math.random()*(i+1)); //generates random numbers from 0-50 + 1 = 52 cards 
            Card temp = deck[i];
            deck[i] = deck[rand];
            deck[rand] = temp;
        }
